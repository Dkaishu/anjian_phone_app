	...
	public Command Login = new Command() {
		...
		public void Invoke(View view, Object... args) {
			...
			Thread th = new Thread(new Runnable() {
				...
				public void run() {
					try {
						String sql = "select ID, NAME, ENAME, charindex((select id from t_role where NAME='维修人员'), roles,1) from t_user where ename='"
		...
		public void handleMessage(Message msg) {
			...
			if (1 == msg.what) {
				...
				try {
					...
					e.printStackTrace();
					Toast.makeText(mContext, "登录失败。请检查用户名和密码是否正确1。", Toast.LENGTH_SHORT).show();
			...
			else if(-1 == msg.what)
			{
				...
				{
					Toast.makeText(mContext, "登录失败。请检查是否联网并确认用户名和密码是否正确2。", Toast.LENGTH_SHORT).show();
					((Button) mContext.findViewById(R.id.button1)).setEnabled(true);
				}
			}
			else  if( 0==msg.what){
				if(pd != null  && pd.isShowing())
					pd.dismiss();
				Toast.makeText(mContext, "登录失败。请检查是否联网并确认用户名和密码是否正确3。", Toast.LENGTH_SHORT)
						.show();
				((Button) mContext.findViewById(R.id.button1)).setEnabled(true);
			}
		}
